...
class ComponentReferenceLinker {
  ...
  _check(doc) {
    let content = doc.contents.toString(); // <(amp-[^\s]+)(?:\s[^>]*)?>([^`]*)?<\/\1>
    // const codeExamples = content.match(/(<(amp-[^\s]+)(?:\s[^>]*)?>(.*?)<\/\2>|```html(.*?)*?```|```css(.*?)*?```|Preview:(.*?)*?<\/amp-\w*(-\w*)*\>|<script(.*?)><\/script>|<!--(.*)?-->|\[sourcecode:\w*](.*?)*?\[\/sourcecode]|<pre>(.*?)\/pre>|(?<= )`\<amp-\w*(-\w*)*(.*?)>`)/gms);
