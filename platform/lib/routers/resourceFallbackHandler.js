...
function createHandler(rootDir) {
   ...
   */
  handler.get(/^(.*\/)([^/]+\.)[a-z0-9]{15,}(\.[^/]+)$/, (request, response, next) => {
    ...
    try {
      ...
      if (availableFile) {
        setNoCache(response);
        log.info('Serve resource with different hash:', request.path, availableFile);
      ...
      log.warn('Error when trying to find fallback resource', request.path, error);
      next();
    }
  });
