...
{% call callout('도움말', type='success') %}
 AMP 에서 요소를 반응형으로 만드는 것은 아주 쉽습니다. `layout="responsive"` 만 추가하면 됩니다. AMP 의 반응형 디자인을 자세히 알아보려면 [반응형 AMP 페이지 만들기](/ko/docs/design/responsive/responsive_design) 를 참조하세요.
{% endcall %}
...
{% call callout('중요', type='caution') %}
 내 스타일이 AMP 에서 지원되는지 확인하세요. 일부 스타일은 성능상의 이유로 지원되지 않습니다([지원되는 CSS ](/ko/docs/design/responsive/style_pages.html) 
참조).
{% endcall %}
...
{% call callout('읽어보기', type='read') %}
  [AMP 에서 지원되는 요소](/ko/docs/design/responsive/control_layout.html) 를 자세히 알아보세요.
{% endcall %}
...
{% call callout('읽어보기', type='read') %}
 자리표시자 및 대체 [자세히 알아보기](/ko/docs/design/responsive/placeholders.html)
{% endcall %}
...
{% call callout('읽어보기', type='read') %}
  [크기 및 srcset으로 아트 디렉션하는 방법](/ko/docs/design/responsive/art_direction.html) 자세히 알아보기
{% endcall %}
...
{% call callout('읽어보기', type='read') %}
 [AMP 페이지 확인 및 오류 수정 ](/ko/docs/fundamentals/validate.html) 방법 자세히 알아보기
{% endcall %}
