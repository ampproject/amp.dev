...
AMP タグに必須の属性は、[AMP 検証ツールの仕様](https://github.com/ampproject/amphtml/blob/master/validator/validator-main.protoascii)に定義されています。
...
このエラーは、属性内のプロパティ値が無効である場合に発生します。この文脈における「プロパティ」という用語は、属性内の構造化された Key-Value データを意味します。たとえば、`<meta name="viewport content="width=device-width;minimum-scale=1">` 内では、`device-width` と `1` がプロパティ値です。
...
スタイルとレイアウトのエラーの説明に移る前に、AMP での[スタイル]({{g.doc('/content/docs/design/responsive/style_pages.md', locale=doc.locale).url.path}})と[レイアウト]({{g.doc('/content/docs/design/responsive/control_layout.md', locale=doc.locale).url.path}})の仕組みを理解しておくことをおすすめします。
...
レイアウトが機能する仕組みについては、[レイアウトを設定する方法]({{g.doc('/content/docs/design/responsive/control_layout.md', locale=doc.locale).url.path}})についての説明や [AMP HTML レイアウト システムの仕様](/docs/reference/spec/amp-html-layout.html)のページをご覧ください。
...
このエラーの原因を把握するには、[各レイアウトの動作]({{g.doc('/content/docs/design/responsive/control_layout.md', locale=doc.locale).url.path}})をよく理解する必要があります。
...
ソースファイルから生成された出力にテンプレートが含まれなければ、ソースファイルにテンプレートを追加してもかまいません（[CSS プリプロセッサの使用]({{g.doc('/content/docs/design/responsive/style_pages.md', locale=doc.locale).url.path}})についての説明もご覧ください）。
...
このエラーは、属性値内に [Mustache テンプレートの構文](https://mustache.github.io/mustache.5.html)が見つかった場合に発生します。
