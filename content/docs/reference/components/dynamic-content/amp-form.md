...
These events can be used through the [`on` attribute](https://github.com/ampproject/amphtml/blob/master/extensions/amp-form/../../spec/amp-html-format.md#on).
...
#### Input Events
AMP exposes `change` events on inputs. This allows you to use the [`on` attribute](https://github.com/ampproject/amphtml/blob/master/extensions/amp-form/../../spec/amp-html-format.md#on) to execute an action on any element when an input value changes.
...
`amp-form` allows publishers to render the responses using [Extended Templates](https://github.com/ampproject/amphtml/blob/master/extensions/amp-form/../../spec/amp-html-format.md#extended-templates).
...
Both success and error responses should have a `Content-Type: application/json` header. `submit-success` will render for all responses that has a status of `2XX`, all other statuses will render `submit-error{% raw %}`{{message}}`{% endraw %}amp-form` also allows publishers to redirect users to a new page after a submission happens through `AMP-Redirect-To` response header.
...
Your XHR endpoints need to follow and implement [CORS Requests in AMP spec](https://github.com/ampproject/amphtml/blob/master/extensions/amp-form/../../spec/amp-cors-requests.md).

### Protecting against XSRF
In addition to following AMP CORS spec, please pay extra attention to [state changing requests note](https://github.com/ampproject/amphtml/blob/master/extensions/amp-form/../../spec/amp-cors-requests.md#note-on-state-changing-requests) to protect against [XSRF attacks](https://en.wikipedia.org/wiki/Cross-site_request_forgery) where an attacker can execute unauthorized commands using the current user session without the user knowledge.
