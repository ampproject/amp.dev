...
Adds support for the [Facebook Pixel](https://www.facebook.com/business/a/facebook-pixel). Requires defining your Pixel ID as 'pixelId': 'YOUR-PIXEL-ID' within the amp-analytics configuration. The events supported along with the corresponding event values that can be specified are detailed at the [Facebook Pixel developer documentation](https://developers.facebook.com/docs/ads-for-websites/pixel-events).
