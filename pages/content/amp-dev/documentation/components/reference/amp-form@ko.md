...
[tip type="important"]
양식의 데이터를 제출하는 경우 서버 엔드포인트에서 [CORS 보안]({{g.doc('/content/amp-dev/documentation/guides-and-tutorials/learn/amp-caches-and-cors/amp-cors-requests.md', locale=doc.locale).url.path}}#cors-security-in-amp)의 요구사항을 구현해야 합니다.
[/tip]
...
[tip type="important"]
XHR 엔드포인트에서 [CORS 보안]({{g.doc('/content/amp-dev/documentation/guides-and-tutorials/learn/amp-caches-and-cors/amp-cors-requests.md', locale=doc.locale).url.path}}#cors-security-in-amp)의 요구사항을 구현해야 합니다.
[/tip]
...
해당 이벤트는 [`on` 속성]({{g.doc('/content/amp-dev/documentation/guides-and-tutorials/learn/spec/amphtml.md', locale=doc.locale).url.path}}#on)을 통해 사용할 수 있습니다.
...
AMP에서는 하위 `<input>` 요소에 `change` 및 `input-debounced` 이벤트를 노출합니다. 따라서 입력 값이 변경될 때 [`on` 속성]({{g.doc('/content/amp-dev/documentation/guides-and-tutorials/learn/spec/amphtml.md', locale=doc.locale).url.path}}#on)을 사용하여 어떤 요소에서도 작업을 실행할 수 있습니다.
...
`amp-form` 확장 프로그램에서 사용자가 [amp-analytics]({{g.doc('/content/amp-dev/documentation/components/reference/amp-analytics.md', locale=doc.locale).url.path}}) 구성에서 추적할 수 있는 다음과 같은 이벤트를 트리거합니다.
...
[확장 템플릿]({{g.doc('/content/amp-dev/documentation/guides-and-tutorials/learn/spec/amphtml.md', locale=doc.locale).url.path}}#extended-templates)(예: [amp-mustache]({{g.doc('/content/amp-dev/documentation/components/reference/amp-mustache.md', locale=doc.locale).url.path}}))을 사용하여 양식에서 성공 또는 오류 응답을 렌더링하거나 [amp-bind]({{g.doc('/content/amp-dev/documentation/components/reference/amp-bind.md', locale=doc.locale).url.path}}) 및 다음 응답 속성을 사용하여 데이터 결합을 통해 성공 응답을 렌더링할 수 있습니다.
...
* [on 속성]({{g.doc('/content/amp-dev/documentation/guides-and-tutorials/learn/amp-actions-and-events.md', locale=doc.locale).url.path}})을 사용하여 양식 *submit-success* 속성을 [`AMP.setState()`]({{g.doc('/content/amp-dev/documentation/components/reference/amp-bind.md', locale=doc.locale).url.path}}#updating-state-with-amp.setstate%28%29)로 결합합니다.
* `event` 속성을 사용하여 응답 데이터를 캡처합니다.
* 원하는 요소에 상태 속성을 추가하여 양식 응답을 결합합니다.

다음 예에서는 [`amp-bind`]({{g.doc('/content/amp-dev/documentation/components/reference/amp-bind.md', locale=doc.locale).url.path}})를 사용하는 양식 `submit-success` 응답을 보여줍니다.
```html
...
허용된 헤더 목록에 `AMP-Redirect-To`가 포함되도록 `Access-Control-Expose-Headers` 응답 헤더를 업데이트하세요.  [AMP의 CORS 보안]({{g.doc('/content/amp-dev/documentation/guides-and-tutorials/learn/amp-caches-and-cors/amp-cors-requests.md', locale=doc.locale).url.path}}#cors-security-in-amp)에서 이 헤더에 관해 자세히 알아보세요.
