...
* Компонент `amp-iframe` может относится к тому же источнику, что и контейнер, только если в атрибуте `sandbox` для них включен элемент `allow-same-origin`. Дополнительные сведения о разрешенных источниках окон iframe доступны в [специальном документе](../../spec/amp-iframe-origin-policy.md).
...
В рекламных целях следует использовать компонент [`amp-ad`](https://www.ampproject.org/docs/reference/components/amp-ad).
...
Мы настоятельно рекомендуем в аналитических целях использовать компонент [`amp-analytics`](https://www.ampproject.org/docs/reference/components/amp-analytics), так как он представляет собой комплексное и эффективное решение, которое можно настроить для самых разных поставщиков аналитических услуг.
...
Компонент `amp-iframe` считается резервным объявлением, если необходимого результата на AMP-странице нельзя добиться другими средствами или если на ней нет подходящего [компонента AMP](https://www.ampproject.org/docs/reference/components). Это связано с тем, что существует ряд преимуществ использования компонента AMP, предназначенного для конкретного случая, например:
