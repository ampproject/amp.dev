...
<tbody>
<tr>
<td>Debe utilizar <code></code> o <code></code> como sus etiquetas de cierre.</td>
...
<tr>
<td>No debe incluir ninguna etiqueta <code><link rel="canonical"></code>.</td>
...
<td>Esos identificadores personalizados pueden utilizarse para identificar tanto a impression como a creative, además pueden ser útiles para informar y eliminar errores.<br><br><p>Por ejemplo:</p> <pre> <meta name="amp4ads-id" content="vendor=adsense,type=creative-id,id=1283474"> </pre>
<meta name="amp4ads-id" content="vendor=adsense,type=impression-id,id=xIsjdf921S">
</td>
</tr>
<tr>
<td> <code><amp-analytics></amp-analytics></code> el seguimiento de visibilidad solo puede orientarse al selector de anuncios completos, mediante <code>"visibilitySpec": { "selector": "amp-ad" }</code> como se define en el <a href="https://github.com/ampproject/amphtml/issues/4018">Issue #4018</a> y <a href="https://github.com/ampproject/amphtml/pull/4368">PR #4368</a>. Particularmente, es posible que no se oriente hacia ningún selector de elementos dentro del anuncio de tipo creative.</td>
<td>Algunas veces, en los anuncios AMPHTML puede selelecionarse renderizar un anuncio de tipo creative en un iframe. En esos casos, el análisis de la página del host solo puede orientar el iframe completo y no tendrá acceso a ningún seleccionador de grano fino.<br><br> <p>Por ejemplo:</p> <pre><br><amp-analytics id="nestedAnalytics">   <script type="application/json">&amp;lt;br&amp;gt;  {&amp;lt;br&amp;gt;    "requests": {&amp;lt;br&amp;gt;      "visibility": "https://example.com/nestedAmpAnalytics"&amp;lt;br&amp;gt;    },&amp;lt;br&amp;gt;    “activa”: {&amp;lt;br&amp;gt;      "visibilitySpec": {&amp;lt;br&amp;gt;      "selector": "amp-ad",&amp;lt;br&amp;gt;      "visiblePercentageMin": 50,&amp;lt;br&amp;gt;      "continuousTimeMin": 1000&amp;lt;br&amp;gt;      }&amp;lt;br&amp;gt;    }&amp;lt;br&amp;gt;  }&amp;lt;br&amp;gt;  </script> </amp-analytics></pre><br> <p>Esta configuración envía una solicitud a la <code>https://example.com/nestedAmpAnalytics</code> URL cuando el 50% del anuncio adjunto pudo visualizarse de forma continua en la pantalla durante 1 segundo.</p></td>
</tr>
</tbody>
</table>

### Código reutilizable <a name="boilerplate"></a>

Los creativos de los anuncios AMPHTML requieren de un código reutilizable diferente y con un estilo considerablemente más simple en comparación con lo que [hacen los documentos generales de AMP](https://github.com/ampproject/amphtml/blob/master/spec/amp-boilerplate.md):
...
*Justificación:* El diseño de `amp-boilerplate` oculta el contenido del cuerpo hasta que el tiempo de ejecución de AMP esté listo y pueda mostrarlo. Si Javascript está deshabilitado o hay un error para cargar para el tiempo de ejecución de AMP, eventualmente el código reutilizable predeterminado garantiza que el contenido se muestre de forma independiente. Sin embargo, si Javascript está completamente deshabilitado en los anuncios de AMPHTML no se ejecutarán y nunca se mostrará ningún anuncio, así que no hay necesidad de utilizar la sección `<noscript>` En ausencia del tiempo de ejecución de AMP la mayoría de la maquinaria de la que dependen los anuncios de AMPHTML no estará disponible (por ejemplo, el análisis para el seguimiento de la visibilidad o `amp-img` para la visualización del contenido), así que es mejor no mostrar ningún anuncio que mostrar uno que no funciona.

Finalmente, el código reutilizable de los anuncios de AMPHTML utilizan  `amp-a4a-boilerplate` en lugar de `amp-boilerplate` para que los validadores puedan identificarlo fácilmente y produzcan mensajes de error más precisos que sirvan de ayuda para los desarrolladores.

Tenga en cuenta que las mismas reglas sobre los cambios en el texto del código reutilizable se aplicaran como en el [código reutilizable general de AMP](https://github.com/ampproject/amphtml/blob/master/spec/amp-boilerplate.md).
...
<tbody>
  <tr>
    <td> <code>position:fixed</code> y <code>position:sticky</code> están prohibidos en el creativo de CSS.</td>
  ...
  <tr>
    <td> <code>touch-action</code> está prohibido.</td>
...
- amp-mraid, es experimental. Si está considerando usarlo, abra un problema en [wg-ads](https://github.com/ampproject/wg-ads/issues/new).
...
Las etiquetas de los SVG no están en el espacio de nombres de HTML5.<br>Se detallan a continuación sin un ID de sección.
