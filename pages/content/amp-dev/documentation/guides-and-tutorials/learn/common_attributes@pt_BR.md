...
Para saber mais, consulte [Consultas de mídia e layout]({{g.doc('/content/amp-dev/documentation/guides-and-tutorials/develop/style_and_layout/control_layout.md', locale=doc.locale).url.path}}) e a [Especificação de layout]({{g.doc('/content/docs/design/amp-html-layout.md', locale=doc.locale).url.path}}).
