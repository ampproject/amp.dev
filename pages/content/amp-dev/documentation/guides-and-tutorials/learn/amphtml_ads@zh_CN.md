...
1. **协调性**：在 AMP 网页上，[AMP runtime]({{g.doc('/content/docs/fundamentals/spec.md', locale=doc.locale).url.path}}#amp-runtime) 可在正确的时间将手机有限的资源协调给正确的组件，以提供最佳的用户体验。例如，如果包含动画的 AMPHTML 广告不在当前视口中，则广告会暂停。
