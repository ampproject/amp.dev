...
CORS 엔드포인트는 `Origin` HTTP 헤더를 통해 출처 요청을 수신합니다. 엔드포인트에서 허용되어야 하는 요청은 다음과 같습니다: (1) 퍼블리셔의 출처, (2) [https://ampjs.org/caches.json](https://ampjs.org/caches.json)에 열거된 모든 `cacheDomain` 출처.
