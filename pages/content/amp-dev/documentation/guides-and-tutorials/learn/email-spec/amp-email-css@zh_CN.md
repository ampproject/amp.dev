...
本文档概述了 AMP 电子邮件内当前支持的 CSS 功能。

## 应用 CSS <a name="applying-css"></a>

### 内部样式表 <a name="internal-stylesheet"></a>

使用内部样式表时，需要在位于电子邮件的 `<head>` 标记中的单个 `<style amp-custom>` 标记内指定 CSS。

### 内嵌样式 <a name="inline-styles"></a>

除了内部样式表之外，还可以使用 [`style` 属性](https://developer.mozilla.org/en-US/docs/Web/HTML/Global_attributes#attr-style)对任何元素指定内嵌样式。

### 外部样式表 <a name="external-stylesheet"></a>

AMP 电子邮件中**不**支持外部样式表。

电子邮件内的任何位置均不允许使用 `<link rel="stylesheet" href="...">`。

## 支持的 CSS 选择器 <a name="supported-css-selectors"></a>
...
## 支持的 CSS 属性 <a name="supported-css-properties"></a>
...
### 其他限制 <a name="additional-restrictions"></a>
...
当前支持的 [@ 规则](https://developer.mozilla.org/en-US/docs/Web/CSS/At-rule)只有 `@media`。请参阅[媒体功能](#media-features)了解详细信息。

### 自定义字体 (`@font-face`) <a name="custom-fonts-font-face"></a>

AMP 电子邮件中**不**支持自定义字体。

## 媒体功能 <a name="media-features"></a>
