...
Este tutorial oferece uma visão geral sobre o componente [`amp-live-list`]({{g.doc('/content/amp-dev/documentation/components/reference/amp-live-list.md', locale=doc.locale).url.path}}) e se concentra em alguns detalhes de implementação dos blogs ao vivo, como a [paginação](#pagination) e os [links diretos](#deeplinking). A [amostra de blog ao vivo](https://www.ampbyexample.com/samples_templates/live_blog/) do AMP By Example será usada para ilustrar a implementação de blogs ao vivo nas AMP.
