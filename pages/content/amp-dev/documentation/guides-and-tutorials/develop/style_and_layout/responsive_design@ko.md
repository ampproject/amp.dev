...
그렇다면 어떤 값을 사용해야 할까요?  AMP에 이미 준비되어 있습니다.  AMP 페이지에 있는 [필수 마크업]({{g.doc('/content/amp-dev/documentation/guides-and-tutorials/learn/spec/index.html', locale=doc.locale).url.path}}#required-markup)의 일부로서 다음 표시 영역을 지정해야 합니다.
...
다양한 유형의 동영상을 AMP 페이지에 추가할 수 있습니다.  자세한 내용은 사용 가능한 [미디어 구성요소]({{g.doc('/content/amp-dev/documentation/components.html', locale=doc.locale).url.path}}) 목록을 참조하세요.
