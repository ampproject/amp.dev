...
The authorization response may be used by AMP Runtime and extensions for two different purposes:
 ...
 2. When evaluating ```<template>``` templates such as ```{% highlight amp-mustache %}```
...
Pingback is configured via {% endhighlight %}pingback``` property in the [AMP Access Configuration][8] section. It is a credentialed CORS endpoint. See [CORS Origin Security][9]” for a list of origins that should be allowed.
