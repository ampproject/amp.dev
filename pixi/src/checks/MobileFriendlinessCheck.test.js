...
describe('Mobile Friendliness check', () => {
  const apiEndpoint = pixiConfig['development'].API_ENDPOINT_MOBILE_FRIENDLINESS;
  ...
  it('returns true for a mobile friendly URL', async () => {
    ...
    const report = await mobileFriendlinessCheck.run('http://mobile-friendly.com');
    expect(report.data).toBe(true);
  });

  it('returns true for a mobile friendly URL that has isues', async () => {
    fetchMock.mock(`begin:${apiEndpoint}`, apiResponse.mobileFriendlyWithIssues);

    const report = await mobileFriendlinessCheck.run('http://a-little-mobile-friendly.com');
    expect(report.data).toBe(true);
  });
  ...
  it('returns false for a mobile unfriendly URL', async () => {
    ...
    const report = await mobileFriendlinessCheck.run('http://mobile-unfriendly.com');
  ...
  describe('returns false if the upstream API', () => {
    ...
    it('could not reach the page', async () => {
      ...
      const report = await mobileFriendlinessCheck.run('http://unreachable.com');
