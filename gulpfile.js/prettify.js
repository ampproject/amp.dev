...
const prettifyGlobs = [
  ...
  '**/*.js',
  '**/*.md',
  '!**/{node_modules,build,dist,dist.3p,dist.tools,boilerplate/templates}/**',
];
...
function runPrettify(filesToCheck) {
  ...
  return new Promise((resolve, reject) => {
    const onData = data => {
      if (!onTravis()) {
        log(green('Checked: ') + path.relative(rootDir, data.path));
      }
    };
    ...
    const onFinish = () => {
      if (!onTravis()) {
        log('Checked ' + cyan(filesToCheck.length) + ' file(s)');
      }
      resolve();
    };
